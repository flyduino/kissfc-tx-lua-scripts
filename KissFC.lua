...
local function postReadFilters(page)
   local filters = {}
   filters[1] = page.values[1] + 1
   filters[2] = page.values[2]
   filters[3] = page.values[3] + 1
   filters[4] = bit32.lshift(page.values[4], 8) + page.values[5]
   filters[5] = bit32.lshift(page.values[6], 8) + page.values[7]
   page.values = filters
end

local function getWriteValuesFilters(values)
   local ret = {}
   ret[1] = bit32.band(values[1]-1, 0xFF)
   ret[2] = bit32.band(values[2], 0xFF)
   ret[3] = bit32.band(values[3]-1, 0xFF);
   ret[4] = bit32.band(bit32.rshift(values[4], 8), 0xFF)
   ret[5] = bit32.band(values[4], 0xFF)
   ret[6] = bit32.band(bit32.rshift(values[5], 8), 0xFF)
   ret[7] = bit32.band(values[5], 0xFF)
   --ret[8] = 0
   return ret
end
...
local SetupPages = {
   ...
   },
   {
      ...
      fields = {
         -- Filters
         { t = "LPF",     x = 15,  y = 25, sp = 30, i=1, min=1, max=7, table = { "Off", "High", "Med. High", "Medium", "Med. Low", "Low", "Very Low" } },
         { t = "Yaw",     x = 15,  y = 38, sp = 30, i=2, min=0, max=97},
         { t = "Notch",   x = 120, y = 25, sp = 60, i=3, min=1, max=2, table = { "Off", "On" }},
         { t = "Center",  x = 120, y = 38, sp = 60, i=4, min=0, max=1000 },
         { t = "CutOff",  x = 120, y = 50, sp = 60, i=5, min=0, max=1000 }
      },
